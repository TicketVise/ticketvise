    ...
    <div class="relative">
      <span class="inline-block w-full rounded-md" @click="open = !open">
        <button aria-expanded="true" aria-haspopup="listbox" aria-labelledby="listbox-label"
                class="h-10 cursor-pointer relative w-full rounded-md border border-gray-300 bg-white dark:bg-gray-800 pl-3 pr-10 py-2 text-left focus:outline-none transition ease-in-out duration-150"
                tabindex="-1"
                type="button">
      ...
      <div class="absolute mt-1 w-full rounded-md bg-white dark:bg-gray-900 dark:border shadow-lg z-50" v-if="open && values && values.length">
        <ul aria-activedescendant="listbox-item-3" aria-labelledby="listbox-label"
          ...
          <li :key="item.id" :value="item.id" @click="switchItem(item)"
              class="text-gray-900 dark:text-gray-200 hover:text-gray-700 hover:bg-primary-400 cursor-pointer select-none relative py-2 pl-3 pr-9"
              id="listbox-item-0"
              role="option"

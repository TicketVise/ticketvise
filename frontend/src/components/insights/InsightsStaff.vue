    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="col-span-1 sm:col-span-2 border rounded-lg px-4 py-2">
        ...
        <ul role="list" class="divide-y divide-gray-200">
          <li
            v-for="member in statsData.staff?.sort(
          ...
          >
            <div class="p-3">
              <div
                class="grid grid-cols-8 gap-2 items-center justify-between w-full"
              >
                <span
                  class="col-span-4 inline-flex items-center px-2 rounded-full text-sm text-gray-800 truncate space-x-2"
                >
                  <img
                  ...
                  <div class="flex flex-col">
                    <span class="truncate text-base font-semibold leading-none"
                      >{{ member.first_name }} {{ member.last_name }}</span
                    >
                    <div class="space-x-2">
                      <span class="text-gray-600 leading-none"
                        >Tickets: {{ member.tickets_count }}</span
                      >
                      <span
                        v-if="member.tickets_count > 0"
                        class="text-gray-600 leading-none"
                        >Response time: {{ member.avg_response_time }}h</span
                      >
                    </div>
                  </div>
                </span>
                <span
                  v-if="member.tickets_count > 0"
                  class="relative col-span-2 inline-flex mr-auto items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-100 text-primary-700 max-w-full"
                >
                ...
                <p
                  v-if="member.tickets_count > 0"
                  class="col-span-2 px-2 inline-flex ml-auto leading-5 font-medium rounded-full text-primary text-xl items-baseline"
                >
                  <span>{{ parseInt(member.helpfulness) }}%</span>
                  <span class="text-gray-400 text-xs font-normal"
                    >/{{ member.amount_of_helpful_comments }}</span
                  >
                  <HandThumbUpIcon class="w-5 h-5 ml-1 self-end" />
                </p>
              </div>
            </div>
          </li>
      ...
      <dl class="flex space-x-4 col-span-1 sm:col-span-2 w-full items-start">
        <div
          v-for="item in stats"
          :key="item.name"
          class="px-4 py-3 rounded-lg border w-full"
        >
...
export default {
  ...
  async mounted() {
    ...
    const statsResponse = await axios.get(
      `/api/inboxes/${inboxId}/statistics/staff`
    );

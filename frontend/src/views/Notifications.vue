  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
    ...
    <div class="grid grid-cols-1 gap-4 items-start">
      ...
      <div class="flow-root mt-6">
        <div
          v-if="notifications.results && notifications.results.length == 0"
          ...
          >
            <div class="flex justify-between space-x-3">
              <div class="min-w-0 flex-1">
                <router-link
                  :to="`/inboxes/${notification.inbox.id}/tickets/${notification.ticket.ticket_inbox_id}`"
                ...
                >
                  ...
                  <p
                    ...
                    :class="
                      notification.is_read ? 'text-gray-400' : 'text-gray-900'
                    "
                  >
              ...
              <time
                :datetime="notification.date_created"
                class="flex-shrink-0 whitespace-nowrap text-sm text-gray-500"
                >{{ date(notification.date_created) }}</time
              >
            </div>
          </li>
        </ul>
        ...
        <nav
          v-show="notifications.next || notifications.previous"
          class="border-t border-gray-200 px-4 flex items-center justify-between sm:px-0"
        >
          <div class="-mt-px w-0 flex-1 flex">
            <button
              v-show="notifications.previous"
              @click="prevPage"
          ...
          <div class="hidden md:-mt-px md:flex">
            ...
            <span
              class="border-primary text-primary-600 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium"
              aria-current="page"
            >
              {{ pageNumber }}
            </span>
            <button
              v-show="notifications.next"
              @click="nextPage"
              class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium"
              aria-current="page"
            >
  ...
  methods: {
    nextPage() {
      if (this.notifications.next) {
        axios
          .get(
            this.notifications.next.substr(
              this.notifications.next.indexOf("/", 7)
            )
          )
          .then((response) => {
            this.notifications = response.data;
    ...
    prevPage() {
      if (this.notifications.previous) {
        if (this.pageNumber === 2) {
          ...
          axios
            .get(
              this.notifications.previous.substr(
                this.notifications.previous.indexOf("/", 7)
              )
            )
            .then((response) => {
              this.notifications = response.data;

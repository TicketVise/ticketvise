      ...
      <li v-for="ticket in pinnedTickets" :key="ticket.id" class="relative col-span-1 flex shadow-sm rounded-md">
        <div class="flex-1 flex items-center justify-between border border-gray-200 bg-white dark:bg-transparent rounded-md truncate">
          <div class="flex-1 px-4 py-2 text-sm truncate">
            <router-link :to="`/inboxes/${$route.params.inboxId}/tickets/${ticket.ticket_inbox_id}`" class="text-gray-900 dark:text-gray-200 font-medium hover:text-gray-600 dark:hover:text-gray-400">
              {{ ticket.title }}
            </router-link>
              ...
              <MenuItems class="z-10 mx-3 origin-top-right absolute right-10 top-3 w-48 mt-1 rounded-md shadow-lg bg-white dark:bg-gray-900 dark:border ring-1 ring-black ring-opacity-5 divide-y divide-gray-200 focus:outline-none">
                <div class="py-1">
                  <MenuItem v-slot="{ active }">
                    <button @click="swapPinTicket(ticket.ticket_inbox_id)" :class="[active ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-300' : 'text-gray-700 dark:text-gray-200', 'group flex w-full items-center px-4 py-2 text-sm']">
                      <PinnedIcon v-if="ticket.is_pinned" class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                      <NotPinnedIcon v-else class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                ...
                <div class="py-1">
                  <MenuItem v-slot="{ active }">
                    <router-link :to="`/inboxes/${$route.params.inboxId}/tickets/${ticket.ticket_inbox_id}`" :class="[active ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-300' : 'text-gray-700 dark:text-gray-200', 'group flex items-center px-4 py-2 text-sm']">
...
import {
  ...
  DotsVerticalIcon,
  ExternalLinkIcon,
  LocationMarkerIcon as PinnedIcon
} from '@heroicons/vue/solid'
import {
  LocationMarkerIcon as NotPinnedIcon
    ...
    getTickets () {
      ...
      axios
        ...
        .then(async (response) => {
          ...
          store.commit('update_public_tickets', { inbox: inboxId, public_tickets: response.data })
        })
    },

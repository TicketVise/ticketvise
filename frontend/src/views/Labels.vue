<template>
  ...
  <div class="mt-3 sm:hidden h-full overflow-y-auto">
    <div class="px-4 sm:px-6">
      <h2 class="text-gray-500 text-xs font-medium uppercase tracking-wide">Labels</h2>
    </div>
    <ul class="mt-3 border-t border-gray-200 divide-y divide-gray-100 border-b">
      <li v-for="label in labels" :key="label.id">
        <router-link :to="`/inboxes/${$route.params.inboxId}/labels/${label.id}`" class="group flex items-center justify-between px-4 py-3 hover:bg-gray-50 sm:px-6">
          <span class="flex items-center truncate space-x-3">
            <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full" :style="`background-color: ${label.color}`" aria-hidden="true" />
    ...
    <div class="w-full flex justify-end p-4">
      <router-link :to="`/inboxes/${$route.params.inboxId}/labels/new`" as="button">
        <button class="bg-primary hover:bg-primary-600 text-white px-4 py-2 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-t border-gray-200">
            ...
            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-800 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider truncate">
              Visible to students
            </th>
          ...
          <tr v-for="label in labels" :key="label.id" class="table-row">
            <td class="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-200">
              <div class="flex items-center space-x-3 lg:pl-2">
                <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full" :style="`background-color: ${label.color}`" aria-hidden="true" />
                <router-link :to="`/inboxes/${$route.params.inboxId}/labels/${label.id}`" class="truncate hover:text-gray-600 dark:hover:text-gray-300">
                  <span>
                    {{ label.name }}
                  </span>
                </router-link>
            ...
            <td class="px-6 py-3 text-sm text-gray-500 font-medium text-center">
              <span v-if="label.is_active" class="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium bg-green-100 text-green-800">
                Active
              </span>
              <span v-else class="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium bg-red-100 text-red-800 truncate">
                In-active
              </span>
            </td>
            <td class="px-6 py-3 text-sm text-gray-500 font-medium flex justify-center">
              <span v-if="label.is_visible_to_guest" class="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium bg-green-100 text-green-800">
                Visible
              </span>
                  ...
                  <MenuItems class="mx-3 origin-top-right absolute right-7 top-0 w-48 mt-1 rounded-md shadow-lg z-10 bg-white dark:bg-gray-900 dark:border ring-1 ring-black ring-opacity-5 divide-y divide-gray-200 focus:outline-none">
                    <div class="py-1">
                      <MenuItem v-slot="{ active }">
                        <router-link :to="`/inboxes/${$route.params.inboxId}/labels/${label.id}`" :class="[active ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-300' : 'text-gray-700 dark:text-gray-200', 'group flex items-center px-4 py-2 text-sm']">
                          <ExternalLinkIcon class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                          Edit label
                        </router-link>
                      </MenuItem>
                    </div>
                    <div class="py-1">
                      <MenuItem v-slot="{ active }">
                        <button @click="deleteLabel(label.id)" :class="[active ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-300' : 'text-gray-700 dark:text-gray-200', 'group flex items-center px-4 py-2 text-sm w-full']">

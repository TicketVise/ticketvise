              ...
              <aside class="mt-8 xl:hidden">
                ...
                <div class="space-y-5">
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <span class="text-gray-900 text-sm font-medium">Created <time :datetime="ticket?.date_created"> {{ date(ticket?.date_created) }}</time></span>
                  </div>
                </div>
          ...
          <section aria-labelledby="activity-title" class="mt-8 xl:mt-10">
            <div>
              ...
              <div>
                ...
                <!-- Attachments -->
                <attachments :ticket="ticket" v-if="tabs.find(t => t.current).name === 'Attachments'" v-on:uploaded="loadTicketData" />
              </div>
            </div>
          </section>
        </div>
                ...
                <span class="relative z-0 inline-flex">
                  <Listbox as="span" class="-ml-px relative block">
                    ...
                    <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                      <ListboxOptions class="absolute z-10 mt-1 bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                        <ListboxOption as="template" v-for="label in ticket?.inbox?.labels" :key="label.id" :value="label" v-slot="{ active }">
                          <li :class="[active ? 'text-white bg-primary' : 'text-gray-900', 'cursor-pointer select-none relative py-2 pl-3 pr-9']">
                            <div class="flex items-center space-x-3">
                              ...
                              <span :class="[label.selected ? 'font-semibold' : 'font-normal', label.avatar ? 'ml-3' : '', 'block truncate']">
                                {{ label.name }}
                              </span>
                            </div>

                            <span v-if="label.selected" :class="[active ? 'text-white' : 'text-primary-600', 'absolute inset-y-0 right-0 flex items-center pr-4']">
        ...
        .then(response => {
          ...
          this.ticket.activity = response.data.events.map(event => {
            ...
            return {
              ...
              person: event.initiator ? { name: event.initiator.first_name + ' ' + event.initiator.last_name, href: '#' } : null,
              tags: type === 'tags' ? [{ name: event.label.name, href: '#', color: event.label.color }] : null,
              assigned: event.assignee ? { name: event.assignee.first_name + ' ' + event.assignee.last_name, href: '#' } : null,

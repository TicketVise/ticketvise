<template>
  <div class="grid grid-rows-[auto_1fr] h-full">
    ...
    <!-- Pages -->
    <form class="p-4 max-w-4xl mx-auto w-full" v-if="tabs.find(t => t.name == 'Appearance').current" @submit.prevent="onSave" method="POST">
      <!-- Inbox section -->
      <div>
        <h2 class="text-lg leading-6 font-medium text-gray-900">Inbox</h2>
        <p class="mt-1 text-sm text-gray-500">
          This information is about the inbox and the look and feel of the inbox. Some settings come direct from your LMS which mean you can only change them there.
        </p>
      </div>

      <div class="mt-6 flex flex-col lg:flex-row">
        <div class="flex-grow space-y-6 mb-4">
          <div>
            <label
              for="name"
          ...
          <div>
            <label
              ...
              class="block text-sm font-medium text-gray-700"
            >
              <span class="mr-1">Visible coordinator</span>
            </label>
            <div class="relative mt-1 flex">
              ...
              >
                <div v-if="inbox.visible_coordinator && staff" class="flex items-center space-x-3">
                  <img
                    :src="staff?.find(s => s.id == inbox.visible_coordinator)?.avatar_url"
                    alt=""
                    class="flex-shrink-0 h-6 w-6 rounded-full"
                  />
                  <span class="block truncate text-gray-800">{{
                    staff?.find(s => s.id == inbox.visible_coordinator)?.first_name + " " + staff?.find(s => s.id == inbox.visible_coordinator)?.last_name
                  }}</span>
                </div>
                <div v-else class="flex items-center space-x-3">
                  <img
                    :src="inbox.coordinator?.avatar_url"
                    alt=""
                    class="flex-shrink-0 h-6 w-6 rounded-full"
                  />
                  <span class="block truncate text-gray-800">{{ inbox?.coordinator?.first_name }} {{ inbox?.coordinator?.last_name }} (default)</span>
                </div>
                <span
              ...
              <div
                class="absolute mt-12 w-full rounded-md bg-white shadow-lg z-10"
                v-if="dropdown_user"
              >
                <ul
                  tabindex="-1"
                ...
                >
                  <li
                    ...
                    :class="{ 'bg-orange-200': user.id == inbox.visible_coordinator }"
                    @click="
                      inbox.visible_coordinator = user.id;
                      dropdown_user = false;
                    "
                  >
                    <div class="flex items-center space-x-3">
                      ...
                      <span
                        ...
                        :class="{ 'font-semibold': user.id == inbox.visible_coordinator }"
                        >{{ user.first_name + " " + user.last_name }}</span
                      >
                    </div>
                    <span
                      class="absolute inset-y-0 right-0 flex items-center pr-4"
                      v-if="user.id == inbox.visible_coordinator"
                    >
                      <svg
    ...
    <form class="p-4 max-w-4xl mx-auto w-full" v-if="tabs.find(t => t.name == 'Tickets').current" @submit.prevent="onSave" method="POST">
      <!-- Privacy section -->
      <div class="divide-y divide-gray-200">
        <div>
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
              These settings are about the tickets and how they are handled.
            </p>
          </div>
    ...
    <div class="p-4 max-w-4xl mx-auto w-full" v-if="tabs.find(t => t.name == 'Labels').current">
      <SettingsLabels />
    </div>
    
    <div class="p-4" v-if="tabs.find(t => t.name == 'Automation').current">
      <SettingsAutomation />
    </div>
    
    <div class="flex flex-1 h-full overflow-hidden" v-if="tabs.find(t => t.name == 'Users').current">
      <SettingsUsers />
    </div>

    <!-- <div class="p-4 max-w-4xl mx-auto" v-if="tabs.find(t => t.name == 'Intelligence').current">

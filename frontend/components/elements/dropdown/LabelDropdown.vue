    ...
    <div class="space-y-1">
            <div class="relative">
                <span class="inline-block w-full rounded-md shadow-sm" @click="open = !open">
                    <button type="button" aria-haspopup="listbox" tabindex="-1" aria-expanded="true" aria-labelledby="listbox-label" class="cursor-pointer relative w-full rounded-md border border-gray-300 bg-white pl-3 pr-10 py-2 text-left focus:outline-none transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                ...
                <div class="absolute mt-1 w-full rounded-md bg-white shadow-lg" v-if="open">
                    <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3" class="max-h-56 rounded-md py-1 text-base leading-6 shadow-xs overflow-auto focus:outline-none sm:text-sm sm:leading-5">
                        <li id="listbox-item-0" :value="item.id" v-for="item in values" :key="item.id" @click="switchItem(item)" role="option" class="text-gray-900 hover:text-white hover:bg-orange-400 cursor-pointer select-none relative py-2 pl-3 pr-9">
                            <div class="flex items-center space-x-3">
